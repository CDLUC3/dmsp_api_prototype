...
Dataset = Literal["crossref", "datacite", "openalex_works"]
...
def keep_record(dataset: Dataset, ror_id: str, record: dict) -> bool:
    if dataset == "openalex":
...
def filter_dataset(dataset: Dataset, ror_id: str, file_in: pathlib.Path, out_dir: pathlib):
        ...
        with gzip.open(file_in, "rt", encoding="utf-8") as f_in:
            for line in f_in:
                if line.strip():
                    ...
                    if keep_record(dataset, ror_id, record):
...
def create_demo_dataset(dataset: Dataset, ror_id: str, in_dir: pathlib.Path, out_dir: pathlib.Path, log_level: int):
    ...
    try:
        with ProcessPoolExecutor(
            ...
            for file_in in files:
                future = executor.submit(filter_dataset, dataset, ror_id, file_in, out_dir)
                futures.append(future)

            total_files = len(files)
            total_filtered = 0
            total_errors = 0
            with tqdm(total=total_files, desc="Filter Dataset", unit="file") as pbar:
...
def setup_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "dataset",
        choices=["crossref", "datacite", "openalex"],
        help="The dataset to filter.",
    )
    parser.add_argument(
        "ror_id",
        type=str,
        help="A ROR ID without a prefix.",
    )
    parser.add_argument(
        "in_dir",
        type=pathlib.Path,
