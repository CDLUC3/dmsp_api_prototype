...
module Functions
  ...
  class RorDownloader
    ...
    NO_HARVESTER_RECORD_MSG = 'No record found for `ror` in the Dynamo ExternalData table!'
    ...
    class << self
      ...
      def fetch_harvester_record(client:, table:, logger: nil)
        key = { RESOURCE_TYPE: HARVESTER_PK, ID: HARVESTER_SK }
      ...
      def update_harvester_record(client:, table:, source:, metadata: nil, tstamp: nil, logger: nil)
        ...
        logger.debug(message: "#{SOURCE} put_item HARVESTER: #{source['ID']}", details: source) if logger.respond_to?(:debug)
        resp
      rescue Aws::Errors::ServiceError => e
        logger&.error(message: format(MSG_DYNAMO_ERROR, msg: e.message, trace: e.backtrace))
        nil
      ...
      def download_ror_file(source:, file_metadata:, tstamp:, logger: nil)
        ...
        key = "#{source['ID']}/#{file_metadata['key']}"
      ...
      def stash_file_in_s3(source:, key:, file:, file_metadata:, tstamp:, logger: nil)
        ...
        bucket = ENV['S3_BUCKET'].gsub('arn:aws:s3:::', '')
        tags = "SOURCE=#{source['ID']}&ID=#{file_metadata['id']}&CHECKSUM=#{file_metadata['checksum']}&DOWNLOADED_ON=#{tstamp}"
      ...
      def alert_processors(source:, file_metadata:, json:, tstamp:, logger: nil)
        ...
        processors.times.each do |proc|
          ...
          events << {
            ...
            detail: {
              zip_file: "#{source['ID']}/#{file_metadata['key']}",
              source: source['ID']&.upcase,
              tstamp: tstamp,
              start_at: last_rec,
