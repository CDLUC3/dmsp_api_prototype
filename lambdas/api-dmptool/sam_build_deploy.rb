...
#   - SSM parameters that are prefixed with: `uc3-dmp-tool-${env}-${Key}` (e.g. `/uc3/dmp/hub/dev/HostedZoneId`)
...
if ARGV.length >= 3
  ...
  if ARGV[1].to_s.downcase.strip == 'true' || ARGV[2].to_s.downcase.strip == 'true'
    ...
    @fetchable_params = [
      ...
      { template_param_name: 'DmptoolApiRoleArn', lookup_name: "uc3-dmp-hub-dmptool-dev-regional-iam-ApiRoleArn" },
      { template_param_name: 'DynamoExternalDataTableName', lookup_name: "#{@cf_export_prefix}DynamoExternalDataTableName" },
